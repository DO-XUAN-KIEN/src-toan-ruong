...
//            add_linh_canh(result, "Thiết kích hộ vệ", i--, 61, j, 5, 2, (short) 100);
    ...
    private static void atk(Map map, Player_Nhan_Ban temp, Player p0) throws IOException {
        int clazz = switch (p0.clazz) {
            case 0 -> 2;
            case 1 -> 4;
            case 2 -> 1;
            case 3 -> 3;
            default -> p0.get_DameBase();
        };
        int dame = p0.get_DameProp(clazz)* 2;
    ...
    public static void player_attack(Session conn, Map map, Player_Nhan_Ban nhanBan, byte index_skill) throws IOException {
        ...
        m_p_2_nb.writer().writeShort(conn.p.index);
        ...
        if (map.map_id != 61 && map.Arena.timeCnBienHinh > System.currentTimeMillis()) {
            dame = 1;
        }
        nhanBan.hp -= dame;
        if (nhanBan.hp <= 0) {
